...
async def test_list_certifications(client, db_session, admin_headers):
    ...
    resp = await client.get(
        f"/api/profiles/{profile.id}/results/certifications", headers=admin_headers
    )
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
...
async def test_list_jobs_filtered_by_run_id(client, db_session, admin_headers):
    ...
    # Filter by run1
    resp = await client.get(
        f"/api/profiles/{profile.id}/results/jobs?run_id={run1.id}", headers=admin_headers
    )
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
    ...
    # Filter by run2
    resp = await client.get(
        f"/api/profiles/{profile.id}/results/jobs?run_id={run2.id}", headers=admin_headers
    )
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
