...
class GoalExtractorOutput(BaseModel):
    ...
    cert_prompt: str = Field(
        description=(
            "Full directive sentence to find vendor certifications,"
            " e.g. 'Search for AWS Solutions Architect and"
            " CompTIA Security+ certifications'"
        )
    )
    course_prompt: str = Field(
        description=(
            "Full directive sentence to find training courses,"
            " e.g. 'Search Coursera and Udemy for Python"
            " and machine learning courses'"
        )
    )
    event_prompt: str = Field(
        description=(
            "Full directive sentence to find events and conferences,"
            " e.g. 'Find 2026 software architecture and"
            " AI conferences in Europe'"
        )
    )
    group_prompt: str = Field(
        description=(
            "Full directive sentence to find professional communities"
            " and groups, e.g. 'Search Discord, Reddit, and LinkedIn"
            " for Java and cloud computing communities'"
        )
    )
    trend_prompt: str = Field(
        description=(
            "Full directive sentence to find trends,"
            " e.g. 'Find emerging trends in cloud-native"
...
class WebScraperOutput(BaseModel):
    ...
    results: list[WebScraperResult] = Field(
        ...
        description=(
            "ALL relevant URLs found during searching. Include even if not verified as live."
        ),
    )
    filtered_urls: list[FilteredURL] = Field(
        default_factory=list,
        description=(
            "Only exact duplicates or clearly irrelevant URLs. Do NOT filter based on liveness."
        ),
    )
